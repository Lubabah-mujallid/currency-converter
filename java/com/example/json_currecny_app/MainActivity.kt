...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {6
        ...
        convertButton.setOnClickListener {
            ...
            getCurrency(onResult = {
                curencyDetails = it
                when {
                    radioButton1.isChecked -> convert(curencyDetails?.eur?.sar?.toDouble(), userInput);
                    radioButton2.isChecked -> convert(curencyDetails?.eur?.usd?.toDouble(), userInput);
                    radioButton3.isChecked -> convert(curencyDetails?.eur?.aud?.toDouble(), userInput);
                    radioButton4.isChecked -> convert(curencyDetails?.eur?.jpy?.toDouble(), userInput);
                }
            })
            }
            else {
